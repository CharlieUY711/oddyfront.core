...
interface Props { canvasRef: React.RefObject<HTMLCanvasElement | null>; onRender?: () => void; }
...
export default function EditCanvas({ canvasRef, onRender }: Props) {
  ...
  const render = useCallback(() => {
    ...
    rafRef.current = requestAnimationFrame(() => {
      ...
      const maxW = w.clientWidth - 16, maxH = w.clientHeight - 16;
  ...
  useEffect(() => { render(); }, [
    ...
    store.blur, store.rotation, store.fineRotation, store.flipH,
    store.flipV, store.zoom, render
  ]);
  ...
  return (
    <div ref={wrapRef} style={{ flex:1, display:"flex", alignItems:"center", justifyContent:"center", overflow:"hidden", position:"relative" }}
      ...
      onDrop={e => { e.currentTarget.style.outline = ""; handleDrop(e); }}>
      {!store.src ? (
        <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:"10px", color:"#666", cursor:"pointer" }}
          onClick={() => document.getElementById("emi-file-input")?.click()}>
          <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#555" strokeWidth="1.5"><rect x="3" y="3" width="18" height="18" rx="3"/><path d="m8 12 4-4 4 4M12 8v8"/></svg>
          <p style={{ fontSize:"12px", textAlign:"center", lineHeight:1.5 }}>Arrastrá o hacé click<br/>para cargar una imagen</p>
          <button style={{ background:"#FF7A00", color:"#fff", border:"none", borderRadius:"7px", padding:"7px 16px", fontSize:"12px", fontWeight:500, cursor:"pointer" }}
